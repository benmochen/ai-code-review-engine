...
from app.core.reviewer import review_diff, preprocess_diff
...
def _format_summary(findings: list[dict], pr_number: int, truncated: bool = False) -> str:
    """Render the findings into a Markdown summary comment for the PR."""
    disclaimer = (
        "\n\n> ⚠️ **Note:** Diff exceeded safety limits and was truncated to 100KB for review. Lockfiles and binary files were omitted."
        if truncated
        else ""
    )
    ...
    if truncated:
        lines.append("")
        lines.append("> ⚠️ **Note:** Diff exceeded safety limits and was truncated to 100KB for review. Lockfiles and binary files were omitted.")
    lines.append("")
...
def process_review(review_id: str) -> dict:
    ...
    try:
        ...
        try:
            cleaned_diff, is_truncated = preprocess_diff(review.diff_text or "")
            findings = review_diff(cleaned_diff)
        except Exception as e:
            review.status = ReviewStatus.FAILED
            db.commit()
        ...
        posted = False
        try:
            summary = _format_summary(findings, review.pr_number, truncated=is_truncated)
            asyncio.run(
